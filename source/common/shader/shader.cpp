...
bool our::ShaderProgram::attach(const std::string &filename, GLenum type) const {
    ...
    if (shader == 0){
        std::cerr<<"ERROR: Shader is not created eith type: " << type << std::endl;
        return false;
    }
    ...
    glDeleteShader(shader); // i could remove it later
...
bool our::ShaderProgram::link() const {
    ...
    if (!error.empty()){
        std::cerr<<"ERROR: Shader program linking Error: " << error << std::endl;
        return false;
    }
