...
uniform vec4 red = vec4(1.0, 0.0, 0.0, 0);
uniform vec4 green = vec4(0.0, 1.0, 0.0, 0);
uniform vec4 blue = vec4(0.0, 0.0, 1.0, 0);

void main(){
    // frag_color = fs_in.color;
    frag_color.r = dot(red, vec4(fs_in.color, 1));
    frag_color.g = dot(green, vec4(fs_in.color, 1));
    frag_color.b = dot(blue, vec4(fs_in.color, 1));
    frag_color.a = 0; // alpha ?
}